...
// GET  /api/deliverables?project_id=&statut=       — liste les livrables d'un projet
router.get('/', deliverablesController.listDeliverables);

// GET  /api/deliverables/pending?project_id=       — livrables en attente (ENCADRANT, JURY)
router.get('/pending', requireRole(ROLES.ENCADRANT, ROLES.JURY), deliverablesController.listPendingDeliverables);

// POST /api/deliverables                           — soumet un livrable (versioning auto)
